...
template <typename TIMx> class Timer {
public:
  ...
  static void stop() { TIMx::regs()->CR1 &= ~(1 << 0); }

  static uint32_t count() { return TIMx::regs()->CNT; }
...
template <> inline void Timer<TIM2_Info>::enableClock() {
