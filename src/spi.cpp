...
void SPI_Driver::init(Mode mode, DataSize dataSize, BaudRate baud) {
    // Enable SPI clock
    if (spi_ == SPI1) {
        *(volatile uint32_t*)0x40023844 |= (1 << 12);  // RCC->APB2ENR |= SPI1EN (bit 12)
    } else if (spi_ == SPI2) {
        *(volatile uint32_t*)0x40023840 |= (1 << 14);  // RCC->APB1ENR |= SPI2EN (bit 14)
    }
